...
class Sports {
    ...
    add(id) {
        let row = '' +
            '<div class="relative">\n' +
            '    <div class="flex items-center text-white bg-yellow-400 rounded-full cursor-pointer transition-all duration-500 absolute -bottom-3 -left-4 hidden add-after" onclick="new AddSection().addComponent({target: this})" data-id="'+(id+1)+'" title="Add component after Sports">\n' +
        ...
        ['en', 'es', 'fr', 'pt'].forEach(lang => {
            ...
            row += '<table data-name="'+(id+1)+'-'+lang+'" class="w-full border-collapse border-2 border-gray-200 rounded-xl shadow-gray-400 shadow-lg '+(lang === 'en' ? '' : 'hidden')+'">\n' +
                ...
                '                <th colspan="4" class="text-base px-1 py-2 text-black font-bold">'+bookmakerName+' '+sport+'</th>\n' +
                '            </tr>';
        ...
        row += '<select class="absolute top-1 -right-11 border-2 border-yellow-400 text-xs rounded language" data-id="'+(id+1)+'" onChange="new Language().toggle({target: this})" title="Change language for Sports">\n' +

...
class Comparison {
    ...
    add(id) {
        let row = '' +
            '<div class="relative">\n' +
            '    <div class="flex items-center text-white bg-yellow-400 rounded-full cursor-pointer transition-all duration-500 absolute -bottom-3 -left-4 hidden add-after" onclick="new AddSection().addComponent({target: this})" data-id="'+(id+1)+'" title="Add component after Сomparison">\n' +
        ...
        ['en', 'es', 'fr', 'pt'].forEach(lang => {
            ...
            '            <span class="text-black font-bold text-sm sm:text-lg px-1 py-2 sm:p-2 w-full">Event:</span>\n' +
            '            <span class="text-black font-bold text-sm sm:text-lg px-1 py-2 sm:p-2 w-24 sm:w-28 flex-shrink-0" id="rate">'+bookmakerName+'</span>\n' +
            '            <select name="'+(id+1)+'-table_v2-'+lang+'-0-2" class="text-lg font-bold text-black w-28 select-bookmaker-2" onchange="new Comparison().changeBookmaker(2, {target: this})" data-id="'+(id+1)+'" title="Select Bookmaker">\n';
            bookmakers.forEach(bookmaker => {
                row += '    <option class="text-black" value="'+bookmaker+'">'+bookmaker+'</option>\n';
            });
            row += '     </select>\n' +
            '            <select name="'+(id+1)+'-table_v2-'+lang+'-0-3" class="text-lg font-bold text-black w-28 select-bookmaker-3" onchange="new Comparison().changeBookmaker(3, {target: this})" data-id="'+(id+1)+'" title="Select Bookmaker">\n';
            bookmakers.forEach(bookmaker => {
                row += '    <option class="text-black" value="'+bookmaker+'">'+bookmaker+'</option>\n';
            });
            row += '     </select>\n' +
            ...
            '            </span>\n' +
            '        </div>\n' +
            '        <div class="flex items-center justify-center self-center text-black bg-yellow-400 rounded-full h-6 w-6 transition-all duration-500 my-1 cursor-pointer add-row-comparison" onclick="new Comparison().addRow({target: this})" data-id="'+(id+1)+'" title="Add Row">+</div>\n' +
            '    </div>\n';
        });
        row += ' <select class="absolute top-1 -right-11 border-2 border-yellow-400 text-xs rounded language" onChange="new Language().toggle({target: this})" data-id="'+(id+1)+'" title="Change language for Сomparison">\n' +

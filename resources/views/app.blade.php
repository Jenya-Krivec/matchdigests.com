...
<head>
    ...
    @foreach(\App\Helpers\Language::LOCALE as $locale => $language)
        ...
        @endif
    @endforeach
    <meta property="og:site_name" content="MATCHDIGESTS"/>
...
<header class="bg-white shadow-sm fixed w-full z-20 border-b-2 border-yellow-400">
    <nav class="flex items-center py-3 w-full">
        ...
        <div class="relative align-self-center ml-auto mr-2">
            <button class="block text-black focus:outline-none" id="language" aria-haspopup="true" aria-expanded="false">
                <span class="hover:bg-yellow-400 p-2 transition-all duration-500">
                    @foreach(\App\Helpers\Language::LOCALE as $locale => $language)
                        @if($locale == app()->getLocale())
                            {{$language}}
                        @endif
...
<div class="fixed -bottom-40 left-0 right-0 bg-transparent z-20 transition-all duration-1000 hidden" id="cookie-message">
    <div class="bg-white w-full lg:w-1/3 mx-auto rounded-md p-6 shadow-lg border-2 border-yellow-400">
        ...
        <div class="flex justify-center mt-4">
            <button class="bg-yellow-400 hover:bg-yellow-300 text-white font-bold py-2 px-4 rounded-full transition-all duration-500" id="cookie-accept">@lang('app.Accept')</button>
...
<footer class="bg-white py-5 border-t-2 border-b-2 border-yellow-400">
    <p class="text-sm text-black text-center font-bold">&copy; {{ date('Y') }} MATCHDIGESTS</p>
</footer>
</body>

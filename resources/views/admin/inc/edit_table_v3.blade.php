<div class="relative">
    <div class="flex items-center text-white bg-yellow-400 rounded-full cursor-pointer transition-all duration-500 absolute -bottom-3 -left-4 hidden add-after" data-id="{{$id}}" title="Add component after Sports">
    ...
    @foreach($values as $lang => $value)
        <table data-name="{{$id.'-'.$lang}}" class="w-full border-collapse border-2 border-gray-200 rounded-xl shadow-gray-400 shadow-lg @if($lang !== 'en') hidden @endif">
            ...
            <tr class="bg-yellow-400">
                ...
                ?>
                <th colspan="4" class="text-base px-1 py-2 text-black font-bold">{{$bookmaker['name'].' '.$sport}}</th>
            </tr>
    ...
    @endforeach
    <select class="absolute top-1 -right-11 border-2 border-yellow-400 text-xs rounded language" data-id="{{$id}}" title="Change language for Sports">

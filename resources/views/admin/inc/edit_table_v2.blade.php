<div class="relative">
    <div class="flex items-center text-white bg-yellow-400 rounded-full cursor-pointer transition-all duration-500 absolute -bottom-3 -left-4 hidden add-after" data-id="{{$id}}" title="Add component after Сomparison">
    ...
    <div data-name="{{$id.'-'.$lang}}" class="flex flex-col justify-center items-center border-gray-200 border-2 rounded-xl shadow-gray-400 shadow-lg @if($lang !== 'en') hidden @endif">
        <div class="flex justify-end items-center bg-yellow-400 w-full rounded-tr-xl rounded-tl-xl">
            <span class="text-black font-bold text-sm sm:text-lg px-1 py-2 sm:p-2 w-full">@lang('review.Event'):</span>
            <span class="text-black font-bold text-sm sm:text-lg px-1 py-2 sm:p-2 w-24 sm:w-28 flex-shrink-0" id="rate">{{$bookmaker['name']}}</span>
            <select name="{{$id.'-table_v2-'.$lang.'-0-2'}}" class="text-lg font-bold text-black w-28 select-bookmaker-2" data-id="{{$id}}" title="Select Bookmaker">
            ...
            <select name="{{$id.'-table_v2-'.$lang.'-0-3'}}" class="text-lg font-bold text-black w-28 select-bookmaker-3" data-id="{{$id}}" title="Select Bookmaker">
                @foreach($bookmaker['bookmakers'] as $otherBookmaker)
        ...
        <div class="flex items-center justify-center self-center text-black bg-yellow-400 rounded-full h-6 w-6 transition-all duration-500 my-1 cursor-pointer add-row-comparison" data-id="{{$id}}" title="Add Row">+</div>
    </div>
    @endforeach
    <select class="absolute top-1 -right-11 border-2 border-yellow-400 text-xs rounded language" data-id="{{$id}}" title="Change language for Сomparison">

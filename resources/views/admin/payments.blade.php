...
@section('content')
    <main class="pt-24">
        ...
        <form action="{{ route('admin.storePayments') }}" method="POST" enctype="multipart/form-data" class="flex justify-center items-center flex-col form">
            ...
            <span class="flex items-center justify-center self-center text-black bg-yellow-400 rounded-full h-6 w-6 transition-all duration-500 mb-2 mx-auto cursor-pointer add-payment" data-id="{{$payment['id']}}" title="Add Payment">+</span>
        </form>
    </main>

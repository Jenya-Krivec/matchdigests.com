<div class="flex justify-between flex-col relative rounded-md text-white p-2 mb-2" style="background: linear-gradient(87deg, {{$bookmaker['logo_color']}} 0%, color-mix(in oklab, {{$bookmaker['logo_color']}}, white 20%) 100%)">
    @if($authorized)
    ...
    <div class="flex justify-between mb-4">
        ...
        <div class="flex justify-center items-center">
            <a href="{{route('review', $bookmaker['key'])}}" class="flex h-10 px-4 text-xs md:text-sm text-center rounded-md justify-center items-center cursor-pointer shadow-gray-400 border-2 border-white mx-2">@lang('index.Read Review')</a>
            <a href="{{route('promocode', $bookmaker['key'])}}" class="sm:flex hidden h-10 px-4 text-xs md:text-sm text-center rounded-md justify-center items-center cursor-pointer shadow-gray-400 border-2 border-white mx-2">@lang('index.Read Promo Code Review')</a>
            <a href="{{route('mobileapp', $bookmaker['key'])}}" class="sm:flex hidden h-10 px-4 text-xs md:text-sm text-center rounded-md justify-center items-center cursor-pointer shadow-gray-400 border-2 border-white mx-2">@lang('index.Read Mobile App Review')</a>
        </div>
        <div class="flex justify-center items-center">
    ...
    <div class="flex justify-between">
        <div class="sm:flex hidden flex-col justify-around self-center h-28 rounded-md p-2 mx-1 md:mx-auto" style="background-color: {{$bookmaker['logo_color']}}">
        ...
        <div class="flex flex-col justify-around self-center min-h-28 sm:h-28 rounded-md p-2 mx-1 md:mx-auto" style="background-color: {{$bookmaker['logo_color']}}">
        ...
        <div class="flex flex-col justify-around self-stretch sm:h-28 rounded-md p-2 mx-1 md:mx-auto" style="background-color: {{$bookmaker['logo_color']}}">
            <div class="text-xs sm:text-sm text-center">@lang('index.Promo code'):</div>
            <div class="flex border-2 border-white rounded w-min pr-2 mx-auto">
